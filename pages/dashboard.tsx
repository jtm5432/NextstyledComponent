...
const Main: React.FC = () => {
  ...
  const [infoBarPosition, setInfoBarPosition] = useState<'right' | 'left' | 'bottom'>('right');
  ...
  const openInfoBar = (content: React.ReactNode, position: 'right' | 'left' | 'bottom') => {
    setInfoBarContent(content);
    setInfoBarPosition(position);
    setIsInfoBarOpen(true);
  };

  // // gridLayout 업데이트 함수
  // const updateGridLayout = (layout: GridLayout[]) => {
  //   const updatedLayout = layout.map(item => {
  //     if (!item.type || item.type === 'honeycomb') {
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <div>
        ...
        <InfoBar content={infoBarContent} isOpen={isInfoBarOpen} onClose={() => setIsInfoBarOpen(false)} position={infoBarPosition} />
      </div>
    </QueryClientProvider>
  );
};
